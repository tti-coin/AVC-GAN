        ...
        for iteration in range(gan_iter):
            ...
            for j in range(d_update):
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                    ...
                    # enc_out = self.model.enc_embedding(batch_x.float().to(self.device), None)
                    # real_rep, attns = self.model.encoder(enc_out, attn_mask=None)
        ...
        self.generator.load_state_dict(
            torch.load(
                os.path.join(
                    ...
                    ae_setting,
                    gan_setting,
                    f"generator_iter{self.args.gan_iter-1}.dat",
                )
            )  # TODO
        )
