"""
デコード後の表現で識別器を学習し、生成器を学習する実験クラス
"""
...
class Exp_iTransGAN(Exp_Basic):
        ...
        for iteration in range(gan_iter):
            ...
            for j in range(d_update):
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                    ...
                    d_real = self.discriminator(torch.squeeze(real_dec))
                    ...
                    d_fake = self.discriminator(fake_dec)
                    ...
                    d_loss = d_fake.mean() - d_real.mean() + reg
                    d_loss.backward()
            ...
            if iteration % 1 == 0:
                print(
                    "[Iteration: %d/%d] [Time: %f] [D_loss: %f] [G_loss: %f] [reg: %f]"
                    % (
                        iteration,
                        gan_iter,
                        time.time() - t1,
    ...
    def eval_gan(self, ae_setting, gan_setting):
        """
        ...
        self.generator.load_state_dict(
            torch.load(
                os.path.join(
                    ...
                    ae_setting,
                    gan_setting,
                    f"generator_iter{self.args.gan_iter-1}.dat", # TODO {self.args.gan_iter-1}
                )
            )
        )
        print("Loading trained max iter WGAN model")
        self.model.eval()
        self.generator.eval()
    ...
    def synthesize(self, ae_setting, gan_setting):
        self.model.load_state_dict(
            torch.load(
                os.path.join("/workspace/checkpoints/", ae_setting, "checkpoint.pth")
            )
        ...
        self.generator.load_state_dict(
            torch.load(
                os.path.join(
                    ...
                    ae_setting,
                    gan_setting,
                    f"generator_iter{self.args.gan_iter-1}.dat", # {self.args.gan_iter-1}
                )
            )  # TODO
        )
        # print("Loading trained === 9999 iter === WGAN model")
        self.model.eval()
        self.generator.eval()
        ...
        f.close()
        # return datas, hiddens

    def wgan_gp_reg(self, x_real, x_fake, center=1.0):
