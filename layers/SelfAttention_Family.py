...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        B, L, H, E = queries.shape # shape(32, 1, 8, 16) # batch, seq_len(=variate), heads, dim
        _, S, _, D = values.shape # shape(32, 1, 8, 16)
        scale = self.scale or 1. / sqrt(E)

        scores = torch.einsum("blhe,bshe->bhls", queries, keys) # shape(32, 8, 1, 1) # batch, heads, variate, variate
        ...
        A = self.dropout(torch.softmax(scale * scores, dim=-1)) # shape(32, 8, 1, 1)
        V = torch.einsum("bhls,bshd->blhd", A, values) # shape(32, 1, 8, 16)
