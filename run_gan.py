...
from experiments.old_exp_sagan import Exp_SAGAN
# from experiments.exp_gan_v2 import Exp_GAN_v2
...
if __name__ == "__main__":
    ...
    # basic config
    parser.add_argument("--is_gan_training", type=int, default=1, help="status")
    ...
    parser.add_argument(
        "--gan_batch_size", type=int, default=32, help="batch size of WGAN"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sample_size", type=int, default=4096, help="sample size of synthetic data"
    )
    ...
    parser.add_argument("--wandb_id", type=str, default="test", help="wandb id")
    parser.add_argument(
    ...
    args.gan_model_id = f"{args.gan_model_id}_{args.des}_pl{args.pred_len}_hd{args.use_hidden}_du{args.d_update}"
    ...
    if args.is_gan_training:
        ...
        if not args.no_wandb:
            ...
            # run_name = f"{args.gan_model_id}_hd{args.use_hidden}_du{args.d_update}"
            if not os.path.exists(os.path.join("/workspace/logs/gan" + gan_setting)):
                os.makedirs(os.path.join("/workspace/logs/gan" + gan_setting))
            wandb.init(
                project="SAGAN",
